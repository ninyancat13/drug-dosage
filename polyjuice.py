...
if (len(sys.argv) < 3):
    ...
    Vinterval = 4   # 4 hours between doses
    Vdosage = 500   # dosage 500mg
else:
    Vinterval = int(sys.argv[1])  # hours between doses
    Vdosage = int(sys.argv[2])    # dosage


half_life = 9.85            # Polyjuice Potion half-life
interval = Vinterval      # Interval between doses
MEC = 55.                 # Effective concentration
MTC = 66.                 # Toxic concentration
...
simulation_time = 500     # simulation time 168
...
print('Polyjuice Potion half-life:\t',half_life)
...
if values.mean() > MEC and values.mean() < MTC and values.max() < MTC:
    plt.figure()
    plt.title('Polyjuice Potion Concentration')
    plt.xlabel('Time (hours)')
    plt.ylabel('Concentration')
    plt.plot(times, values, '-', times, MECline, 'g-', times, MTCline, 'r-')
    #plt.plot(values)
    plt.savefig('Polyjuice_dosage_' + 'I' + str(Vinterval)+'_D' + str(Vdosage) + '.png')
    print('\nRESULTS\n')
